...
const formIniziale: FormStateInterface = {
  protagonista: "",
  antagonista: "",
  genere: "",
};
...
const Form = () => {
  const [formState, setFormState] = useState<FormStateInterface>(formIniziale);
  ...
  const handleInputChange = (key: keyof FormStateInterface, value: string) => {
    setFormState((prev) => ({
      ...prev,
      [key]: value,
    }));
  ...
  const clean = () => {
    setFormState(formIniziale);
    setPrompt("");
  };

  const promptGenenretor = async () => {
    setLoading(true);
    if (process.env.NEXT_PUBLIC_GEMINI_API_KEY && useAI) {
      ...
      const result = await model.generateContent(
        `creami una storia con protagonista ${formState.protagonista} e antagonista ${formState.antagonista} in stile ${formState.genere}. aggiungi sempre il titolo`
      );
  ...
  return (
    <div className={styles.form}>
      <div className={styles.box_input}>
        <Input
          label="Inserisci Protagonista"
          value={formState.protagonista}
          action={handleInputChange}
          stateKey="protagonista"
        />
        <Input
          label="Inserisci Antagonista"
          value={formState.antagonista}
          action={handleInputChange}
          stateKey="antagonista"
        />
        <Select
      ...
      <div className={styles.box_button}>
        <span>
          start
          <TbHandClick
            onClick={promptGenenretor}
            className={styles.button_icon}
          />
        </span>
        <span>
          clean
          <LuPaintbrush onClick={clean} className={styles.button_icon} />
        </span>
        ...
        {isReading ? (
          <>
            <span>
              stop
              <FaRegStopCircle onClick={stop} className={styles.button_icon} />
            </span>
            <span>
              pause
              <FaRegPauseCircle
          ...
          <span>
            speak
            <GiSpeaker onClick={readStory} className={styles.button_icon} />
          </span>
        )}
        ...
        {showResume && (
          <span>
            resume
            <GrResume onClick={riprendi} className={styles.button_icon} />
          </span>
        )}
      </div>
      {loading && <ImSpinner className={styles.button_icon} />}

      <div className={styles.box_prompt}>{prompt}</div>
    </div>
  );
};
